...
#
#   This file is part of python-plyfile     此文件是python-plyfile的一部分，也是封装的模块，用来import的.
#
...
def make2d(array, cols=None, dtype=None):
    ##从数组数组中创建一个二维数组。cols 和 dtype如果为空，则参数可以省略
    '''
...
class PlyParseError(Exception):
    #plyparseerror   (在无法解析PLY文件时引发。)
    '''
    Raised when a PLY file cannot be parsed.在无法解析PLY文件时引发。
...
class PlyData(object):
    #PLY文件标题和数据
    #PlyData实例可以通过以下两种方式之一来创建：通过静态方式方法PlyData.read（读取PLY文件），
    # 或直接从__init__#给定一系列元素（然后可以写入PLY#文件）。
    '''
    PLY file header and data.PLY文件标题和数据
...
class PlyElement(object):
    #PLY文件元素
    #该库的客户端通常不需要直接实例化，因此以下内容仅用于记录内部。
    #创建PlyElement实例通常以两种方式之一完成：作为PlyData.read的副产品（读取PLY文件时）和PlyElement.describe（在编写PLY文件之前）
    '''
    PLY file element.
...
class PlyProperty(object):
    #PLY属性描述。这个类是纯粹的元数据;数据本身包含在PlyElement实例中。
    '''
